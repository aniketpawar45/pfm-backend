...
class TelegramService:
    ...
    @classmethod
    async def process_update(cls, body: dict):
        if "callback_query" in body:
            ...
            if data.startswith("del_page_"):
                ...
                keyboard = cls.build_delete_keyboard(records, total_pages, page, query_arg, selected_ids)
                cls.edit_message(chat_id, message_id, f"🗑️ **{title}**\nSelect transactions to delete:", keyboard)
                ...
                cls.edit_message(chat_id, message_id, f"✅ Successfully deleted {deleted_count} transaction(s).")
                
            elif data == "del_cancel":
                DBService.clear_user_selections(chat_id)
                cls.edit_message(chat_id, message_id, "❌ Deletion cancelled.")
        ...
        if text_stripped.startswith("/start"):
            ...
            cls.send_message(chat_id, welcome_msg)

        elif text_stripped.startswith("/setsalary"):
            parts = text_stripped.split()
            if len(parts) < 2:
                cls.send_message(chat_id, "⚠️ Usage: `/setsalary [amount]` (e.g., `/setsalary 75000`)")
            else:
                try:
                    amount = float(parts[1])
                    DBService.set_user_salary(chat_id, amount)
                    cls.send_message(chat_id, f"✅ Base monthly salary successfully updated to **{amount:,.2f}**.")
                except ValueError:
                    cls.send_message(chat_id, "⚠️ Invalid amount format. Please provide a valid number.")
            ...
            if len(parts) < 6:
                ...
                try:
                    ...
                    cls.send_message(chat_id, f"⚠️ Error adding loan: {str(e)}")

        elif text_stripped.startswith("/loans"):
            loans = DBService.get_user_loans(chat_id)
            if not loans:
                cls.send_message(chat_id, "ℹ️ You have no active or recorded loans.")
            else:
            ...
            if not categories:
                cls.send_message(chat_id, "ℹ️ No expense data available to generate chart for this period.")
            else:
                ...
                cls.send_photo(chat_id, tmp_path, caption=f"📊 Expense Chart: {title}")
                try:
                    os.unlink(tmp_path)
                except:
                    pass
            ...
            if not records:
                cls.send_message(chat_id, "ℹ️ No transactions found to export.")
            else:
                import csv
                import io
                output = io.StringIO()
                ...
                cls.send_document(chat_id, csv_bytes, filename, caption=f"📁 Transaction Statement: {title}")
            ...
            if not records:
                cls.send_message(chat_id, f"ℹ️ No transactions found for '{title}'.")
            else:
                keyboard = cls.build_delete_keyboard(records, total_pages, 0, query_arg, selected_ids)
                cls.send_message(chat_id, f"🗑️ **{title}**\nSelect transactions to delete:", keyboard)

        else:
            parsed = AIService.parse_transaction(text_stripped)
            if not parsed or not parsed.get("is_transaction", True):
                cls.send_message(chat_id, "🤖 I didn't quite catch that. You can log expenses, extra income, or pay EMIs naturally!")
            else:
