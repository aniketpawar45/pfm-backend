...
# Add your database client imports here (e.g., Supabase client setup)
...
class TelegramService:
    ...
    async def process_natural_language(cls, chat_id: int, text_input: str = None, audio_bytes: bytes = None):
        try:
            ...
            # Handle non-financial chatter / ignored messages
            if isinstance(parsed_data, dict) and not parsed_data.get("is_transaction", True):
                await cls.send_message(chat_id,
                                       "🤖 I'm your PFM assistant. Send me expenses or income notes (e.g., 'Spent 500 on lunch') or grocery lists to log them!")
                return

            # Format confirmation message back to user
